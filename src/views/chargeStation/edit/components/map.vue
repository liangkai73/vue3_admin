...
<template>
    <div class="inner-box">
        ...
        <div class="flex_r_s mt20">
            <el-input type="text" v-model="lng" placeholder="经度" style="width: 250px;"></el-input>
            <el-input type="text" v-model="lat" placeholder="纬度" style="width: 250px;"></el-input>
...
const props = defineProps({
    stationLng: { type: Number, require: true },
    stationLat: { type: Number, require: true },

});

const emit = defineEmits(['update:stationLng', 'update:stationLat']);

(window as any)._AMapSecurityConfig = {
    securityJsCode: "ced1865fc8f2d748912c68526eddb3dd",
};
...
let lng = computed({
    get() {
        return props.stationLng
    },
    set(e) {
        emit('update:stationLng', e)
    }
})
let lat = computed({
    get() {
        return props.stationLat
    },
    set(e) {
        emit('update:stationLat', e)
    }
})

onMounted(() => {
    AMapLoader.load({
        key: "9cb26ec5ab68d00b2878fd64e0a0f8a2", // 申请好的Web端开发者Key，首次调用 load 时必填
        ...
        .then((AMap) => {
            let center: any = '';
            let marker: any = '';
            if (props.stationLat != 0) {
                center = [props.stationLng, props.stationLat]
                marker = new AMap.Marker({
                    position: new AMap.LngLat(props.stationLng, props.stationLat),
            ...
            placeSearch.on('markerClick', (e: any) => {
                lng.value = e.event.lnglat.lng;
                lat.value = e.event.lnglat.lat;
            })
